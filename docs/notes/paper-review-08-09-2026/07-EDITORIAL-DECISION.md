...
- **No bit-security level is stated for any parameter set.** Raised by R2 F14 as MAJOR and by R1 F12 as a reproducibility failure. Verified, memo item 8, as corrected on 2026-09-08: the string 128 occurs zero times in the submission, and its three occurrences in the report are all the citation [128] for CryptPEFT. Neither document states a bit-security level, so this is a gap in the package and not only in the ten pages. R2's judgement is that this is the single thing a reader needs in order to believe the cryptography.
...
| B10 | No bit-security level, total modulus, or secret distribution is stated for any parameter set. Every theorem is stated with negl(lambda) and lambda is never instantiated. | R2 F14 MAJOR, R1 F12 MAJOR, editor-verified memo 8 | One line per chain: ring degree, log Q, scale, secret distribution, claimed classical bit security, and the standard table or estimator used. Say which of these moves if B1's smudging fix needs a larger scale. | WRITING. The justification exists in `fhe/serve.go`: the serving chain is QP approx 762 bits at ring degree 2^15, inside the published 128-bit budget for that degree. Editor's note, 2026-09-08: the parameters are custom `ckks.ParametersLiteral` chains, not Lattigo presets, so the paper must state the chain and cite the table rather than name a preset. The one part not recorded anywhere is the total QP of the bootstrapping configuration, where Lattigo's default bootstrapping literal adds moduli on top of the residual chain. That one may need the estimator. |
