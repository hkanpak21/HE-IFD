\section{Why Fine-Tuning, not Training from Scratch}
...
Blindness is a sharper constraint than cost. A homomorphic server evaluates a fixed arithmetic circuit chosen
...
supplies the frame for nothing. Freezing the backbone fixes the representation every client sees, so each
