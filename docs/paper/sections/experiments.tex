...
standard label-skew partition: a smaller $\alpha$ leaves each client a few dominant classes and almost
none of the rest. Unless stated otherwise there are $\Nc=10$ clients, the adapter has rank $8$, the
...
candidate hold the 77-class Banking77 gap to $0.11$, down from over $0.5$ for the naive average. HE-OFT
...
likelihood-ratio attack LiRA, under an external adversary and under the
\emph{fellow-client} adversary that brings its own data as a prior, the strongest the threat model of
\cref{sec:threat} admits (\cref{tab:mia}). On the text tasks other than Banking77 the leakage is at the
level of random
...
$100$-class CIFAR-100, LiRA is likewise mildly elevated \textcolor{red}{[TODO: vision MIA numbers pending
the re-run on the freeze-$A$ ViT]}, consistent with the same coverage effect at a large label space.

A finding worth stating explicitly is that the fellow-client prior yields \emph{no} advantage over the
external adversary: the two columns of \cref{tab:mia} coincide. This is not a coincidence of our data but a
property of the attack. LiRA already calibrates each example against the empirical distribution of shadow
models that did and did not train on it, so the per-class knowledge a fellow client could add ---
essentially a shift of the score's location by class --- is exactly what LiRA has already conditioned away,
and it cancels in the likelihood ratio. A participant's extra knowledge helps a cruder loss-threshold
attack, but on the calibrated attack it buys nothing, so being a participant rather than an outsider does
not increase what one can learn about another's membership from the released model. A participant therefore
learns little about another's membership from the shared model beyond what any usable
...
its own data as a prior. $0.50$ AUC and $1\%$ TPR are chance. The two adversaries coincide because LiRA's
per-example shadow calibration already absorbs the fellow's class prior (see text).}
\label{tab:mia}
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
CIFAR-100 (100) & \textcolor{red}{??} & \textcolor{red}{??} & \textcolor{red}{??} & \textcolor{red}{??} \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
reports the result. At the data-free DENSE setup (CIFAR-10, $\Nc=5$) HE-OFT reaches $0.96$ against their
...
\begin{table}[t]
\centering
\caption{HE-OFT at the published partitions of prior one-shot methods (frozen ViT-B/16 $+$ freeze-$A$
