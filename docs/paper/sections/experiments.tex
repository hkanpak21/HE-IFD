...
\caption{HE-IFD lifts the single encrypted aggregation well above the chance start and above the naive
...
Two effects compound in \cref{tab:headline}. First, fine-tuning the frozen backbone's own layers with a
small adapter lifts the model far above the chance start: from a $\thz$ that classifies at chance to between
...
candidate set with client-side selection (\cref{sec:candidates}) lifts the released model well above the
naive sample-weighted average, by $+0.24$ on AG-News, $+0.21$ on TREC, $+0.13$ on DBpedia and $+0.38$ on
Banking77; the naive average is exactly the combination that the heterogeneity makes fragile, and the vote
recovers the candidate that survives it. The remaining gap to the centralized fine-tune is small at moderate
...
large-norm Gaussian, or one trained on label-flipped data. The honest clients still vote on their local
holdouts, now over the plain aggregate and the $\Nc$ leave-one-out aggregates; the candidate that excludes
the attacker wins whenever excluding it helps.
...
% ============================================================================
% COMMENTED OUT until jobs/dp_baseline.py lands its numbers (PI comment 2026-06).
% Centralized DP-SGD logistic head on the SAME frozen RoBERTa features, eps grid,
% as the lossy privacy-utility ceiling our zero-cost crypto point sits above.
% Fill acc cells from results/dp_baseline/cell_*.json, then uncomment.
% \begin{table}[t]
% \centering
% \caption{Differential privacy is lossy where we are not. A centralized DP-SGD
% logistic head on the same frozen features (the \emph{best case} for DP) versus
% HE-IFD's released model, which carries no privacy-induced noise. Accuracy at
% $\delta=10^{-5}$; HE-IFD column repeats \cref{tab:headline}.}
% \label{tab:dp}
% \footnotesize
% \begin{tabular}{lccccc c}
% \toprule
% & \multicolumn{5}{c}{DP-SGD head (centralized)} & HE-IFD \\
% \cmidrule(lr){2-6}
% Task & $\varepsilon{=}1$ & $2$ & $4$ & $8$ & non-priv. & (ours) \\
% \midrule
% AG-News    & \tbd & \tbd & \tbd & \tbd & \tbd & 0.75 \\
% TREC       & \tbd & \tbd & \tbd & \tbd & \tbd & 0.72 \\
% DBpedia    & \tbd & \tbd & \tbd & \tbd & \tbd & 0.93 \\
% Banking77  & \tbd & \tbd & \tbd & \tbd & \tbd & 0.77 \\
% \bottomrule
% \end{tabular}
% \normalsize
% \end{table}
