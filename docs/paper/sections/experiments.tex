...
Banking77 (77) --- with frozen RoBERTa-base and MPNet encoders, and on CIFAR-100 with a frozen ViT. Data
...
Alongside the accuracy of the HE-OFT global model we report two reference points: a frozen-feature linear
probe trained under the same protocol, and, as the ceiling, the same adapter and head fine-tuned centrally
on the pooled private data, what one party would obtain holding all of it. The quantities of interest are
the \emph{increment} over the probe and the \emph{gap} below the centralized ceiling; the centralized
...
\midrule
\textbf{HE-OFT (ours)} & multiparty CKKS & adapter$+$head & \textbf{1} & \textbf{$9.5$ MiB} \\
