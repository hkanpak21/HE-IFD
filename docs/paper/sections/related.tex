...
their rounds cell is empty. POSEIDON keeps the model under the collective key and
states that the querier does not obtain the model's weights.}
\label{tab:related}
\footnotesize
...
\Cref{tab:related} places the closest systems on the axes that separate them. One prior scheme is one-shot, federated and encrypted, for a
closed-form learner only~\cite{fedheonn2023}. What remains unaddressed is a protocol that
