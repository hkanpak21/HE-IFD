...
survey draws that separation explicitly~\cite{kairouz2021advances}. Against models that generalize well they identify few members, and only at
...
and against models that generalize well they identify few members, and only at
low false-positive rates~\cite{carlini2022membership}. The reason for the gap
is simple. An update is a detailed view of one client's data and arrives every
round, where the final model is a single average that generalization moves away
...
That cost scales with the network being trained, therefore adapting a pretrained
...
All of them remain bound to the iterative protocol, hence the encryption cost is paid
...
These share our structural niche, but they run an optimizer on ciphertexts, hence multiplicative depth grows with
the step count.}
...
unit, but they run an optimizer on ciphertexts, thus multiplicative depth grows with
...
is added to the artifact that is released. Several
...
peers, and their privacy is lossy, because the noise a meaningful budget requires
is added to the artifact that is released. Differential privacy proves a
statistical bound on how much a released artifact can reveal, and it pays for that bound
...
FFA-LoRA instead freezes the randomly initialized down-projection and trains only
the up-projection, hence averaging the trained factor is
...
FFA-LoRA instead freezes the randomly initialized down-projection and trains only
the up-projection, hence averaging the trained factor is
exact~\cite{sun2024ffalora}. Combining displacements from a shared initializer is
...
cost~\cite{zhang2025nexus}. The map served here is linear in the features, thus serving a
trained map placed inside the backbone instead would reduce to exactly the
...
is what makes the disclosure setting of \cref{sec:serving} affordable, and we compose with it. The map served here is linear in the features, thus serving a
trained map placed inside the backbone instead would reduce to exactly the
problem these systems address.}

\paperonly{Two systems are closer. slytHErin evaluates a network under CKKS with the model
...
homomorphically, and the models it reports are therefore a twenty-layer and a
fifty-layer network on a handwritten-digit task rather than a pretrained encoder.
And it returns the prediction vector, hence the querier receives scores, which
\cref{sec:exp-leak} shows is a cheaper target for extraction than a
label.}

\tronly{Two systems are closer. slytHErin evaluates a network under CKKS with the model
held under a multiparty key, and it key-switches the prediction to the querier,
which is the serving setting of \cref{sec:serving}~\cite{intoci2023slytherin}. The
same group built the encrypted training scheme of \cref{sec:related-crypto}, therefore
...
encryption, and the client receives the adapter's
output~\cite{xia2026cryptpeft}. It is a
...
client encrypts the resulting features, only the adapter is evaluated under
encryption, and the client receives the adapter's
output~\cite{xia2026cryptpeft}. Confining encrypted computation to a small unit
after the representation is the motivation for our design as well. It is a
two-party protocol, the trained unit is the provider's plaintext, and there is no
training protocol, therefore it establishes that the serving arrangement is practical
...
adapter is one party's plaintext, hence it evaluates no ciphertext-by-ciphertext
...
carries the collective refreshes that the threshold setting requires, thus neither
transfers unchanged.}
...
train, hence their rounds cell is empty. POSEIDON keeps the model under the
...
CryptPEFT~\cite{xia2026cryptpeft}    & many & 2PC  & provider & scores \\
\midrule
