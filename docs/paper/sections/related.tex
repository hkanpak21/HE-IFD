...
Five lines of work bear on this paper: the leakage properties of federated
learning, the cryptographic tools that protect training, one-shot federated
learning and its differentially private variants, federated fine-tuning of
adapters, and secure inference. We take them in that order, since the first
establishes the premise on which the design rests and the remainder determine its
position. The closing paragraph returns to the claim of \cref{sec:intro} and
identifies what existing work leaves unaddressed.

\subsection{Leakage at Training Time and at Inference Time}
...
established that these training-time quantities constitute the most damaging
disclosure such a protocol makes.
...
The asymmetry admits a straightforward explanation. Training-time quantities are
...
the protection. The threshold structure of \cref{sec:mhe} discharges that assumption rather than
imposing it.
...
the parties that built it. The third of these is not a refinement of the first two.
