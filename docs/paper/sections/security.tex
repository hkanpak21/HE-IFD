...
Three properties of the construction close the routes a coalition could take.
Every adapter stays with its owner, and the protocol never forms the aggregate a
coalition would subtract its own contributions from. The shared head stays a
ciphertext throughout, which closes the same route against it. The server inverts
the per-class totals under encryption and never decrypts them. A coalition cannot
...
displacements are ciphertexts, and the merged head is a ciphertext. Nothing is
...
measure the protocol. The server sits outside $\Fhe$, and the semi-honest adversary of
\cref{sec:threat} may corrupt it. The form of the functionality follows the
...
circuit that do not depend on any plaintext. The simulator knows their number
...
public. An adversary free to choose datasets of different sizes reads the
...
$\tc-1$ shares. The server is honest. The values it computes are the
prescribed deterministic functions of those ciphertexts and of public scalars.
The server is honest. Every ciphertext presented for key switching is
...
\Cref{thm:malicious} keeps the server honest. No protocol with this message
pattern can drop that assumption. Decryption is a protocol the clients run on a ciphertext the server presents. A
...
backbone with the adapter of client $j$, and that client is corrupt.
...
changes what is ultimately learnable. Both are decisions about cost.}
...
\cref{prop:c2c} receives both shared heads in plaintext. We give an attacker
...
measuring there. No party ever holds either head. This is the world in which
...
\tronly{The strongest label-only attack we found does no better, and it fails for
a reason worth naming. OSLO crafts one query per candidate rather than reading a
statistic off a model, and it reports a true-positive rate of $6.7$ per cent at a
false-positive rate of $0.001$ on CIFAR-100 where the previous best label-only
attack reports $0.3$~\cite{peng2024oslo}. Our serving interface is more exposed to
it than an ordinary one, because a client uploads its own encrypted feature vector
and \cref{thm:malicious} lets a coalition deviate in the queries it asks. The
crafted point enters directly. Over eighteen cells the attack reaches an area
under the curve between $0.47$ and $0.52$.}

\tronly{The reason sits in the attack's own calibration. A coalition chooses its
one perturbation on data it holds, where it knows for every surrogate which
examples that surrogate trained on. The best separation it achieves there is
$0.065$ at most and below $0.01$ in most cells. The crafted query does not fail to
transfer to the served head. The difference in margin between a member and a
non-member is not there to transfer.}
...
class a client holds. Held-out examples are richer in rare classes than
...
pattern fixed. They say nothing about how leakage moves when the partition
moves.}

\tronly{Read together, the three surfaces say one thing. Even where extraction
has already succeeded, and even where we hand the head over outright, the
membership attack of Carlini et al.~\cite{carlini2022membership} stays near
chance. The head is a linear map fitted for two hundred steps over thousands of
examples per class, and the coverage-weighted merge gives a coalition no row it
can read.}
...
key switch that moves the ciphertext off the collective key. The key it
...
corrupts the server together with $\tc-1$ clients. This view is inside the
model rather than outside it. Resubmitting a query is also what the protocol
permits. The adversary is semi-honest.}
...
label. The precision the answer needs is a handful of bits against a plaintext
