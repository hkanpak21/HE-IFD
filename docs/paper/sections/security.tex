...
scalars. The phase signals sit there because anyone watching the network sees them.
We prove security relative to both.
...
transcript is simulatable by the security of the key-generation protocol.
...
clients deviate, and \cref{sec:malicious-ext} shows why the server must stay
honest. We bound what a deviating coalition learns about the data of a client
...
\Cref{sec:exp-leak} measures $\delta$ and turns it into a bound on $Q$.

\subsection{Necessity of an Honest Server}
\label{sec:malicious-ext}

\Cref{thm:malicious} keeps the server honest. That assumption is necessary,
and the reason is worth stating, because it marks the boundary of what this
message pattern can achieve.

Decryption is a protocol the clients run on a ciphertext the server presents. A
...
restoration is deterministic as well, because the protocol restores levels under
collectively generated bootstrapping keys rather than by collective refresh. A
...
queries per parameter for a functional copy. This is a property of the
functionality, not of our instantiation of it. Any protocol that realizes $\Fhe$
inherits it.
...
on a deployment's ability to meter queries.
