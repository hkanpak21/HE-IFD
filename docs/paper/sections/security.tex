...
\cref{sec:exp-cost} would move with them. We report the configuration we
measured and state the gap rather than claim a guarantee we did not instantiate.}

\tronly{One further axis deserves a countermeasure even though it does not
threaten the allowance. The serving circuit is deterministic, so a client that
resubmits an identical query obtains the same ciphertext while every honest
client draws fresh smudging noise. Averaging enough such repetitions reduces the
noise on the shares until the value beneath it can be read off. The number of
repetitions is between $10^5$ and $10^8$ depending on which key-switch protocol
runs, against an allowance two to five orders of magnitude smaller, so the axis
is not reachable at the budgets we consider. It closes completely at no cost by
refusing to key-switch the same ciphertext twice, which the server enforces by
storing a hash of every ciphertext presented. We recommend it and we do not rely
on it.}
