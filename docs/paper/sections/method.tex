...
\begin{algorithm}[t]
\caption{HE-OFT: one-shot federated fine-tuning under multiparty CKKS, with the two disclosure settings.}
\label{alg:heoft}
\small
\begin{algorithmic}[1]
\Statex \textbf{Public, shared, data-free:} frozen backbone $\featx$; initializer $\thz$ of the trainable unit (freeze-$A$ adapter $+$ head)
\State clients run distributed key generation $\to$ collective CKKS key \Comment{secret-shared; no party decrypts alone}
\For{client $j=1,\dots,\Nc$ \textbf{in parallel}}
  \State $\theta_j \gets \textsc{FineTune}(\thz,\Dj,\Ksteps)$ \Comment{plaintext, on the client}
  \State send $\mathsf{Enc}(\disp)$ to the server, \; $\disp=\theta_j-\thz$
\EndFor
\State \textbf{server} forms depth-one candidates $\{\mathsf{Enc}(\theta^{(m)})\}$ from $\{\mathsf{Enc}(\disp)\}$: scaling $\thz+\lambda\!\sum_j\wj\disp$, count-/Fisher-reweighted, leave-one-out \Comment{$+$ and public-scalar $\times$ only}
\State clients threshold-decrypt the candidates and select $\thstar\!\gets\!\theta^{(m^\star)}$, $m^\star=\arg\max_m \overline{\mathrm{acc}}_m$ on local holdouts
\Statex \rule{\linewidth}{0.4pt}
\Statex \textbf{Disclosure of $\thstar$} (\cref{sec:serve}):
\State \textbf{Release:} threshold-decrypt $\thstar$ $\to$ every client holds the plaintext model
\State \textbf{Serve:} keep $\mathsf{Enc}(\thstar)$; on query $x$: $\mathsf{Enc}(\ell)\!\gets\!\mathsf{Enc}(\thstar)\,\featx(x)$ (depth $1$); $\hat y\!\gets\!\arg\max_c \ell_c$ under encryption; threshold-decrypt $\hat y$ only
\end{algorithmic}
\end{algorithm}
...
plaintext to features $\featx(x)$, and the encrypted head produces encrypted logits by the very operation the
aggregation uses,
\begin{equation}
\label{eq:serve}
\mathsf{Enc}(\ell) \;=\; \mathsf{Enc}(\thstar)\,\featx(x),
\qquad
\hat{y} \;=\; \arg\max_{c\in\{1,\dots,\Cc\}}\ell_c ,
\end{equation}
a plaintext vector against ciphertext weights at multiplicative depth one, so no nonlinearity ever meets the
encrypted head. The $\arg\max$ is taken \emph{under encryption} by a homomorphic comparison
circuit~\cite{cheon2020comparison}, and a threshold of clients decrypts the predicted label $\hat{y}$ alone.
...
$\lceil\log_2\Cc\rceil$; our multiparty implementation measures this at under two minutes for a hundred
classes, a fourteenfold reduction and exact, and single-key GPU systems built on the same reduction report a
full-vocabulary $\arg\max$ in well under a second~\cite{zhang2025nexus}. None of this is specific to the threshold setting: homomorphic evaluation under a
