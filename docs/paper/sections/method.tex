...
the ability to read its inputs, by moving the choice to the clients after decryption. The observation we use is that several
useful aggregation rules are each, on their own, a depth-one linear function of the encrypted displacements,
so the server can evaluate all of them in one pass and the clients select among the results. We form a small set of
