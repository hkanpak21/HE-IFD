...
units, $\thstar = \sum_j \wj\, \theta_j^{(\Ksteps)}$. That linearity is what keeps the operation inexpensive to
...
because in our one-shot protocol the server never obtains a plaintext sum at all. Robustness to actively
malicious or Byzantine clients is outside our scope; we discuss it as a direction for future work in
\cref{sec:scope}.
