% method.tex -- restructured 2026-09-09 on Sinem's comments, following the shape
% of CURE (PoPETs 2026): an unnumbered lead of Problem Statement, Threat Model
% and Motivation, then a named Overview, then numbered subsections whose
% third-level headings name their content.
%
% The version the PIs read before this is docs/archive/method-pre-restructure-2026-09-09.tex.
...
and its own adapter in plaintext, and the shared trained quantity cannot be
placed inside the backbone. A server computing on ciphertexts cannot inspect a value or
...
trained quantity that is shared cannot be placed inside the backbone. One further
...
of it. Only the head is shared. Client $j$ encrypts the displacement
$\disp = h_j - \thz \in \mathbb{R}^{\Cc\times d}$ of its head from the public initializer and uploads it once. The server
...
$\mathrm{diag}(g_j)\,\disp$ locally, before encryption, thus the server only adds
ciphertexts.
...
\tronly{Two properties of that circuit should be stated, because a deployment has
to size them. The inversion converges on a bounded positive interval, and we
scale the totals by the federation's example count, which is public, so the
argument lands in $(2^{-6},1]$. A class whose share falls below $2^{-6}$ is
outside that interval. Our measurement uses denominators in $[0.02,0.99]$ and
reports a relative error of $1.9\times10^{-8}$ at ten parties, so it does not
cover the low end. Banking77 and CIFAR-100 fall there, at class shares near
$0.013$ and $0.010$, and a deployment at those label-space sizes must widen the
interval or rescale by the largest class total rather than by the example count.}

\tronly{A class that no client holds would divide by zero, and detecting that
under encryption is a comparison rather than a scaling. We do not implement one.
\Cref{sec:exp-split} counts the holders of every class row and finds none at zero
on any task, seed or partition we measured, so the case does not arise in our
configuration. A deployment that cannot rule it out needs either a public list of
covered classes, which discloses coverage, or a comparison in the aggregation
circuit, which the cost of \cref{sec:exp-cost} does not include.}
...
features. Serving a trained quantity placed inside the backbone is the problem
...
quantity has none, thus the restriction gains less here than they suggest. It
...
vote. That procedure estimates the wrong quantity. It selects the personal
arrangement in every cell, so \cref{sec:exp-select} finds it wrong wherever the
