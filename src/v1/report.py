...
CASE_BLURB = (
    ...
    "what the encrypted version produces."
)


def write_csv(path: Path, cells: List[CellResult]) -> None:
    fieldnames = [
        "method", "dataset", "N", "alpha", "seed", "K", "tau", "probe_size",
        "student_acc", "mean_teacher_acc",
...
def render_table(cells: List[CellResult]) -> str:
    ...
    head = ("| N | seed | student_acc | mean_teacher_acc | wall_clock (s) | status |\n"
            "|---|------|-------------|------------------|----------------|--------|")
    body_lines = []
    for N, seed, sa, mt, wc, status in rows:
        sa_s = f"{sa:.4f}" if sa is not None else "n/a"
        mt_s = f"{mt:.4f}" if mt is not None else "n/a"
        body_lines.append(f"| {N} | {seed} | {sa_s} | {mt_s} | {wc:.1f} | {status} |")
    return head + "\n" + "\n".join(body_lines)
...
def write_report(results_dir: str, cells: List[CellResult], args: Dict) -> None:
    ...
    config_block = (
        ...
        f"- Probe size: `{args.get('probe_size')}`\n"
