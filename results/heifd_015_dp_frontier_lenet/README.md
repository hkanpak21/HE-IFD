...
from-scratch LeNet-5 / FashionMNIST backbone (issue 015). **Blocked-but-prepared:**
...
Does the averaging-variant DP frontier **flatten from ε ≈ 2 onward** on a conv
backbone? The probe releases per-(client, class) means under the Gaussian
mechanism with σ = (clip / Kpc) · √(2 ln(1.25/δ)) / ε, so:
...
- Backbone: `lenet_fmnist` (from-scratch LeNet-5 on raw 1×28×28 FashionMNIST)
- N values: `10`
